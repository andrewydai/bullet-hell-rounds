...
enum PowerupType { SHOOT, SPREAD, TRAP, TURRET }
...
## Spawns a randomly chosen pickup (SHOOT or SPREAD) at a random screen position.
func _spawn_pickup() -> void:
	...
	# Set the type before add_child so _ready() draws the correct sprite
	var pickup: Node = (load(PICKUP_SCENE_PATH) as PackedScene).instantiate()
	pickup.powerup_type = PowerupType.SHOOT if randf() < 0.5 else PowerupType.SPREAD
